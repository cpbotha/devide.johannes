...
# Francois Malan - added Clinical Graphics Patch
# All rights reserved.
...
#This patch contains added functionality by Clinical Graphics
CG_POLY_PATCH = "vtk58_PolyDataConnectivityFilter_clinicalgraphics.diff"
...
class VTK58CG(InstallPackage):
    ...
    def __init__(self):
        ...
        self.cg_poly_patch_src = os.path.join(config.patches_dir, CG_POLY_PATCH)
        self.cg_poly_patch_dst = os.path.join(config.archive_dir, CG_POLY_PATCH)
    ...
    def get(self):
        ...
        if not os.path.exists(self.exc_patch_dst):
            ...
            # CG_POLY PATCH
            utils.output("Applying ClinicalGraphics PolyDataFilter patch")
            os.chdir(self.source_dir)
            # default git-generated patch, so needs -p1
            ret = os.system(
                "%s -p1 < %s" % (config.PATCH, self.cg_poly_patch_filename))
            if ret != 0:
                utils.error(
                    "Could not apply ClinicalGraphics PolyDataFilter patch.  Fix and try again.")
